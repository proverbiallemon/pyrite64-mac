...
# === Tiny3D ===
# Always rebuild after libdragon — ucode/API changes can break tiny3d
if [ -e "tiny3d" ]; then
...
# Build Tiny3D
if [[ ! -f "$sdkpath/bin/gltf_to_t3d.exe" || "${FORCE_UPDATE:-}" == "true" || -n "${LIBDRAGON_PIN:-}" ]]; then
