...
bool Editor::ToolchainOverlay::draw()
{
  ...
  ImGui::SetNextWindowSize({850, 550}, ImGuiCond_Always);
  ...
  ))
  {
    ...
    if(!ctx.toolchain.isInstalling())
    {
      if(allDone)
      {
        ...
        // differs from the latest commit (HtD pulled the brake)
      ...
      // Install/Update button (for not-fully-installed state)
      if(!allDone) {
        ImGui::SetCursorPos({
